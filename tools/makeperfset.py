...
# Set is taken from https://github.com/attractivechaos/plb/blob/master/sudoku/sudoku.txt
...
def main():
    ...
    for probnum, line in enumerate(SET.split('\n')):
        if not line:
            continue
        rawprob = line[:81].replace('.', '0')
        with open(os.path.join(pdir, '%02d.txt' % probnum), 'w') as f:
            for ln in range(9):
                sline = rawprob[ln*9:ln*9+9]
                print(' '.join(list(sline)), file=f)
        print("Generated problem %02d.txt" % probnum)
